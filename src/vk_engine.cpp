...
void VulkanEngine::draw()
{
    ...
    if (presentResult == VK_ERROR_OUT_OF_DATE_KHR) {
        resize_requested = true;
        // no return here, as it is at the end of the function.
    }
...
void VulkanEngine::draw_geometry(VkCommandBuffer cmd) {
    ...
    // for the perspective, near and far are flipped to improve depth testing.
...
AllocatedBuffer VulkanEngine::create_buffer(size_t allocSize, VkBufferUsageFlags usage, VmaMemoryUsage memoryUsage)
{
    ...
    // VMA_ALLOCATION_CREATE_MAPPED_BIT used for mapping the pointer automatically.
...
GPUMeshBuffers VulkanEngine::uploadMesh(std::span<uint32_t> indices, std::span<Vertex> vertices)
{
    ...
    // STORAGE_BUFFER = SSBO
    // VK_BUFFER_USAGE_TRANSFER_DST_BIT as we are going to perform memory copy commands.
    ...
    //create index buffer
    // VK_BUFFER_USAGE_INDEX_BUFFER_BIT for indexed calls.
    // VK_BUFFER_USAGE_TRANSFER_DST_BIT as we are going to perform memory copy commands.
    newSurface.indexBuffer = create_buffer(indexBufferSize, VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
        VMA_MEMORY_USAGE_GPU_ONLY);

    // The targetted buffers are created with the flag VMA_MEMORY_USAGE_GPU_ONLY for fastest access.
    // However, it is not readable and therefore a staging buffer is created to allow to copy the data to the fast memory.
    // VK_BUFFER_USAGE_TRANSFER_SRC_BIT as is the only use is a copy command.
    ...
    // With this method the CPU wait on the completion of the GPU copy command.
    // To improve, offload this task to a background task.
    immediate_submit([&](VkCommandBuffer cmd) {
